...
st.set_page_config(
    page_title="Gem Value | Premium Diamond Price Predictor",
    page_icon="💎",
    layout="centered",
    initial_sidebar_state="expanded"
)

# 2. Fixed Custom CSS (Ensures all labels and text are fully visible)
st.markdown("""
    <style>
    ...
    /* FIX: Force all input labels, slider text, and headers to be bright and clear */
    .stSlider label, .stSelectbox label, label, .stMarkdown h3 {
    ...
    /* Fix minor helper or small text within widgets */
    .stSlider [data-testid="stWidgetMarkdownClaims"] p {
    ...
    .stButton>button:hover {
        ...
        background: linear-gradient(45deg, #64dfdf, #7400b8);
        color: #ffffff !important;
    }
...
# 4. App Headers
st.markdown('<div class="main-title">💎 GEMVALUE PREDICTOR</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">AI-Powered Premium Diamond Valuation Engine</div>', unsafe_allow_html=True)

if model is None:
    st.error("⚠️ Error: 'best_diamond_pricing_model.pkl' file not found! Please upload it to your repository.")
else:
    # Interactive features: Estimator Info Accordion
    with st.expander("✨ How are Diamond Prices Calculated? (The 4 Cs Info)"):
        st.write("""
    ...
    st.markdown('### 🛠️ Configure Diamond Specifications')
    
    # 5. Organizing Inputs using interactive sliders and dropdowns
    col1, col2 = st.columns(2)
    
    with col1:
        carat = st.slider("⚖️ Carat Weight", min_value=0.2, max_value=5.5, value=1.0, step=0.01)
        cut = st.selectbox("🎬 Cut Quality", ["Ideal", "Premium", "Very Good", "Good", "Fair"])
        color = st.selectbox("🎨 Color Grade (D is best, J is lowest)", ["D", "E", "F", "G", "H", "I", "J"])
        clarity = st.selectbox("🔍 Clarity Grade", ["IF", "VVS1", "VVS2", "VS1", "VS2", "SI1", "SI2", "I1"])

    with col2:
        depth = st.slider("📐 Depth Percentage (%)", min_value=43.0, max_value=79.0, value=61.5, step=0.1)
        table = st.slider("🪟 Table Width (%)", min_value=43.0, max_value=95.0, value=57.0, step=0.1)
        x = st.slider("📏 Length (x) in mm", min_value=0.0, max_value=12.0, value=6.4, step=0.1)
        y = st.slider("📐 Width (y) in mm", min_value=0.0, max_value=59.0, value=6.4, step=0.1)
        z = st.slider("💎 Depth (z) in mm", min_value=0.0, max_value=31.0, value=4.0, step=0.1)
    ...
    if st.button("✨ VALUATE DIAMOND PRICE"):
        with st.spinner("⚡ Running AI Appraisal Engine..."):
            time.sleep(1.2)
